...
| Public API | All Domain Model types; `validateDocument()`; `migrate()` contract shapes; `ValidationResult`; and the **structural ports** this ring declares for contracts it consumes from outer rings — `WidgetTypeLookup`, `WidgetDataValidator`, `WidgetDataMigrator` (ADR-0013). Revised 1.4.2: this row previously claimed the `WidgetDefinition` interface itself, which Ring 0 cannot hold — Widget-System.md §3's shape carries an adapter-facing `render` member and a `hitTest` whose `Point` type is defined nowhere in the corpus. `WidgetDefinition` is `presentation-widget-api`'s |
...
| 1.4.2 | `presentation-domain`'s Public API row corrected: Ring 0 declares structural ports (`WidgetTypeLookup`, `WidgetDataValidator`, `WidgetDataMigrator`), not outer-ring contracts — the previous claim to hold `WidgetDefinition` was not implementable | ADR-0013 |
